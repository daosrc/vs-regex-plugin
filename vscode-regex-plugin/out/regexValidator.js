...
class RegexValidator {
    ...
    async testRegex() {
        ...
        try {
            ...
            if (matches) {
                vscode.window.showInformationMessage(`匹配成功! 匹配结果: ${JSON.stringify(matches)}`, '详细信息').then(selection => {
                    if (selection === '详细信息') {
                        vscode.window.showInformationMessage(`完整匹配: ${matches[0]}\n` +
                ...
                vscode.window.showWarningMessage(`匹配失败。输入文本: "${testText}", 正则表达式: /${pattern}/`);
            }
        }
        catch (error) {
